...
class C1C0System(System):
    ...
    serial_threads: Mapping[str, SerialReader]
    controller_thread: XBoxControllerReader = XBoxControllerReader()
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.thread_list.extend(self.serial_threads.values())
        ...
        def get_data(data_id):
            ...
            for t in self.thread_list:
                if getattr(t, 'data_id') == data_id:
                    return getattr(t, 'data')

        def start_subsystem(caller, target):
            with self.active_subsystems_lock_suite.writer():
                ...
                subsystem = subprocess.Popen([
                    os.path.join('.', 'shells', 'start.sh'), target, caller
                ])
