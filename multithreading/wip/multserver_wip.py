...
def threaded_client(connection):
    ...
    while(getattr(t, "do_run", True) and detectClient):
        ...
        if(data.decode('utf-8') == "I am xboxcontroller"):
            ...
            detectClient = False
            connection.sendall(str.encode(reply))
        elif(data.decode('utf-8') == "I am stop-xboxcontroller"):
            reply = "stop-xboxcontroller is recognized"
            client = "stop-xboxcontroller"
            detectClient = False
            connection.sendall(str.encode(reply))
        elif not data:
            break
    ...
    while(getattr(t, "do_run", True) and (not detectClient)):
        ...
        elif (client == "stop-xboxcontroller"):
            if ("xbox" in data.decode('utf-8')):
                #print(data)
                #print(data.decode('utf-8')[6:])
                reply = "xboxcontroller signal: " + "(+0.00, +0.00)" + " sent to arduino"
                Locomotion_API.locomotion_msg("(+0.00, +0.00)") # serial port: /dev/ttyTHS1 USB port: /dev/ttyACM0
                connection.sendall(str.encode(reply))
            elif("head" in data.decode('utf-8')):
                #print(data)
                #print(data.decode('utf-8'))
                reply = "xboxcontroller signal: " + "(0, 0, 1)" + " sent to arduino" #check the format of data
                headrotation.head_msg("(0, 0, 1)") # serial port: /dev/ttyTHS1 USB port: /dev/ttyACM0
                connection.sendall(str.encode(reply))
            elif("strong" in data.decode('utf-8')):
                #print(data)
                reply = "xboxcontroller signal: " + "[3, 3, 3, 3]" + " sent to arduino"
                strongarm.strong_msg("[3, 3, 3, 3]")
                connection.sendall(str.encode(reply))
            elif("precise" in data.decode('utf-8')):
                #print(data)
                reply = "xboxcontroller signal: " + "[0, 0, 0, 0, 0, 0, 0]" + " sent to arduino"
                precisearm.arm_msg("[0, 0, 0, 0, 0, 0, 0]")
                connection.sendall(str.encode(reply))
